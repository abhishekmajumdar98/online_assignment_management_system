<?php
	...
	$query = 'SELECT * FROM st_details WHERE id = ' . $_SESSION['std_id'];
...
<?php
	if(isset($_POST['update']))
	{
		...
		$dob=$_POST['dob'];
			...
			{
				$sql= 'UPDATE `st_details` SET `name`="'.$name.'",`email`="'.$email_id.'",`contact`="'.$mob_no.'",`course`="'.$course.'",`address`="'.$address.'",`year`="'.$year.'", `dob`="'.$dob.'" WHERE id='.$_SESSION['std_id'];
				mysqli_query($con,$sql);
				echo '<script language="javascript">';
				echo 'alert("UPDATE SUCCESSFULL");';
				$_SESSION['std_user_name']=$email_id;
				$_SESSION['std_name']=$name;
				echo 'window.location = "student_home.php";';
				echo '</script>';
			}
	}
?>